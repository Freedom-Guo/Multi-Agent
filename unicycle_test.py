...
while is_all_capture(Rc):
    ...
    for i in range(pursuer_num):
        ...
        af_points[i+evader_num-len(death)] = [af_points[i+evader_num-len(death)][0] + V_p[i]*math.cos(pursuer[i]), af_points[i+evader_num-len(death)][1] + V_p[i]*math.sin(pursuer[i])]
